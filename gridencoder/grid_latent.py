...
class GridEncoder(nn.Module):
    def __init__(self, input_dim=3, input_features_dim = 256, num_levels=16, level_dim=2, per_level_scale=2, base_resolution=16, log2_hashmap_size=19, desired_resolution=None, gridtype='hash', align_corners=False, interpolation='linear'):
        super().__init__()
        ...
        self.latent_embeddings_1 = nn.Linear(input_features_dim, (offset * level_dim) // 2, bias=False)
        self.latent_embeddings_2 = nn.Linear((offset * level_dim) // 2, offset * level_dim, bias=False)
    ...
    def forward(self, inputs, bound=1):
        ...
        inputs = (inputs[:,-3:] + bound) / (2 * bound) # map to [0, 1]
        latent_1 =  self.latent_embeddings_1(inputs[:,-3])
        latent_2 =  self.latent_embeddings_2(latent_1).view(latent_1.shape[0], latent_1.shape[1]//2, 2)
        #print('inputs', inputs.shape, inputs.dtype, inputs.min().item(), inputs.max().item())

        prefix_shape = list(inputs.shape[:-1])
        inputs = inputs.view(-1, self.input_dim)
        
        outputs = grid_encode(inputs, latent_2, self.offsets, self.per_level_scale, self.base_resolution, inputs.requires_grad, self.gridtype_id, self.align_corners, self.interp_id)
